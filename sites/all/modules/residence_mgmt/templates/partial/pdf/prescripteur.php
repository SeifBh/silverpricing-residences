...
#https://ehpad.home/sites/all/modules/residence_mgmt/templates/partial/pdf/prescripteur.php
if(strpos($_SERVER['REQUEST_URI'],'partial/pdf/prescripteur.php') and is_file($f)){
...
foreach( $historyResult->response as $k=>$residence ) {
    ...
    if (isset($historyResult->organismes->$finess)) {
        foreach ($historyResult->organismes->$finess as $t) {#Persons
            ...
            echo $t['titre'].' '.$t['nom'].' '.$t['prenom'].' '.$t['fonction'].' '.$t['service'].' '.$t['telephone'].' '.$t['email'].' '.$t['ville'];
            echo"</td></tr>";
            $a = 1;
        }
    }
